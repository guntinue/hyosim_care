...
- [x] 고객 등록 및 관리 화면
  ...
  - [x] 고객 목록 조회 화면
    - [x] QTableWidget 기반 목록 표시
    - [x] 컬럼: ID, 이름, 생년월일, 연락처, 주소, 서비스유형, 등급, 상태
    - [x] 활성/비활성 고객 구분 표시
  - [x] 고객 검색 기능
    - [x] 이름, 전화번호, 서비스유형별 검색
    - [x] 전체 보기 기능
  - [x] 고객 관리 기능
    - [x] 신규 등록 (PatientForm 연동)
    - [x] 수정 (더블클릭 또는 수정 버튼)
    - [x] 비활성화 (삭제 대신)
- [ ] 직원 등록 및 관리 화면
- [ ] 캘린더 형태의 일정 관리 뷰
...
- **2026-04-23 오전**: 고객 등록 화면 구현
  ...
  - 고객 등록 버튼 이벤트 핸들러 구현
  - 애플리케이션 실행 테스트 성공
- **2026-05-02 오후**: 고객 목록 조회 화면 구현
  - feature/ui-patient-list 브랜치 생성
  - PatientListDialog 다이얼로그 구현
    - QTableWidget 기반 목록 표시
    - 8개 컬럼 (ID, 이름, 생년월일, 연락처, 주소, 서비스유형, 등급, 상태)
    - 활성/비활성 고객 색상 구분 (회색 처리)
  - 검색 기능 구현
    - 이름, 전화번호, 서비스유형별 검색
    - 검색 타입 선택 (QComboBox)
    - 전체 보기 버튼
  - 고객 관리 기능
    - 신규 등록 (PatientForm 연동)
    - 수정 (더블클릭 또는 수정 버튼)
    - 비활성화 (확인 다이얼로그 포함)
  - MainWindow에 "고객 목록" 버튼 추가
  - 애플리케이션 실행 테스트 성공
- **Next Step**:
  - 직원 등록 화면 구현 (feature/ui-staff-form)
  - 직원 목록 조회 화면 구현 (feature/ui-staff-list)
  - 메인 대시보드에 실시간 통계 연동
- **현재 브랜치**: `feature/ui-patient-list`
