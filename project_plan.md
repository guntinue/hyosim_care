...
- [x] 고객 등록 및 관리 화면
  ...
  - [ ] 고객 검색 기능
- [x] 직원 등록 화면
  - [x] StaffForm 다이얼로그 구현
  - [x] 기본 정보 입력 (이름, 역할, 연락처, 이메일, 입사일)
  - [x] 자격 정보 입력 (자격증 번호)
  - [x] 추가 정보 입력 (메모)
  - [x] 유효성 검증 (필수 필드 체크)
  - [x] StaffService 연동 (등록/수정)
  - [x] MainWindow 연동 (직원 등록 버튼)
- [ ] 직원 목록 조회 화면
- [ ] 캘린더 형태의 일정 관리 뷰
- [ ] 방문 일지 작성 화면
- [ ] 통계 및 리포트 화면
...
- **2026-04-23 오전**: 고객 등록 화면 구현
  ...
  - 고객 등록 버튼 이벤트 핸들러 구현
  - 애플리케이션 실행 테스트 성공
- **2026-05-02 오후 (2차)**: 직원 등록 화면 구현
  - feature/ui-staff-form 브랜치 생성
  - StaffForm 다이얼로그 구현
    - 기본 정보 입력 (이름, 역할, 연락처, 이메일, 입사일)
    - 자격 정보 입력 (자격증 번호)
    - 추가 정보 입력 (메모)
    - 유효성 검증 로직 (필수 필드 체크)
    - StaffService 연동 (등록/수정 기능)
    - 스타일시트 적용 (PatientForm과 일관성 유지)
  - MainWindow에 StaffService 연동
  - 직원 등록 버튼 이벤트 핸들러 구현
  - 애플리케이션 실행 테스트 성공
- **Next Step**:
